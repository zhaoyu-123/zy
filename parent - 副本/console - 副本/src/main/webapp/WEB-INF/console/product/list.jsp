...
<div class="body-box">
<form action="/product/list.action" method="post" style="padding-top:5px;">
名称: <input type="text" name="name"/>
	<select name="brandId">
		<option value="">请选择品牌</option>
		<c:forEach items="${brandList }" var="brand">
			<option value="${brand.id }" <c:if test="${brandId == brand.id }">selected</c:if>>${brand.name }</option>
		</c:forEach>
	</select>
	<select name="isShow">
		<option value="1" <c:if test="${isShow }">selected</c:if>>上架</option>
		<option value="0" <c:if test="${!isShow }">selected</c:if>>下架</option>
	</select>
...
<table cellspacing="1" cellpadding="0" width="100%" border="0" class="pn-ltable">
	...
	<c:forEach items="${pagination.list }" var="product">
		<tr bgcolor="#ffffff" onmouseover="this.bgColor='#eeeeee'" onmouseout="this.bgColor='#ffffff'">
			...
			<td align="center">
			<a href="#" class="pn-opt">查看</a> | <a href="#" class="pn-opt">修改</a> | <a href="#" onclick="if(!confirm('您确定删除吗？')) {return false;}" class="pn-opt">删除</a> | <a href="/sku/list.action?productId=${product.id }" class="pn-opt">库存</a>
			</td>
		</tr>
		</c:forEach>
	</tbody>
